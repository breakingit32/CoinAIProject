...
def locate_coin(img):
    ...
    for i in circles[0, :]:
        ...
        # Ensure X, Y, W, H won't overun the edge fo the image
        if x < 0:
            x = 0
        if y < 0:
            y = 0
        ...
        # Ensure X2, Y2, W2, H2 won't overun the edge fo the image
        if x2 < 0:
            x2 = 0
        if y2 < 0:
            y2 = 0
...
start_url = 'E:/PROJECTS/CentSearch/Sorted_Images/Obverse_Lincoln/date/'
file_count = 0  # Increment the file count as you iterate through the directory

start_url = pathlib.Path(start_url)
image_count = len(list(start_url.glob('*/*.jpg')))
print(image_count)
...
for root, dirs, files in os.walk(start_url):
    for file in files:
        if file.endswith('.jpg'):
            ...
            current_image = cv.imread(os.path.join(root, file), 1)  # Filename in this directory
            ...
            for x in range(0, 360, 1):
                ...
                cv2.imwrite(f"E:/PROJECTS/CentSearch/training/obverse_lincoln/"
                            f"angle/obverse_linc_{x}/{file_count}_obverse_lincoln_{x}.jpg",
                            rotated)

cv.imshow('Final Image', rotated)  # Shows the final image rotated.
